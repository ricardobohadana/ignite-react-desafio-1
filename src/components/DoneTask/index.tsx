...
interface TaskProps extends ITask {
    ...
    handleOnClickDoneCheckbox: (id: string) => void;
}

export function DoneTask({id, done, content, handleOnClickDoneCheckbox, handleOnClickDelete}: TaskProps) {

    return (
      <main className={style.mainDone}>
        <aside className={style.mainTaskContent}>
          ...
          <p className={style.doneContent}>
            {content}
          </p>
        </aside>
        <aside
          className={style.deleteTask}
          onClick={(e) => handleOnClickDelete(id)}
        >
          <svg
            width="24"
            height="24"
